...
let DaySlot = React.createClass({
  ...
  renderEvents() {
    let {
      date, events, step, min, culture, eventPropGetter
    ...
    return events.map((event, idx) => {
      ...
      if (!dates.default.eq(end, date, 'day'))
          endSlot = positionFromDate(dates.default.subtract(dates.default.add(date, 1, 'day'), 1, 'seconds'), min, step);
      if (!dates.default.eq(start, date, 'day'))
